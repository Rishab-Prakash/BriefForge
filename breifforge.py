...
def export_to_pdf(brief_text):
    ...
    h1 { font-size: 18pt; color: #1a365d; margin: 0 0 8px 0; border-bottom: 2px solid #3182ce; }
    h2 { font-size: 14pt; color: #2c5aa0; margin: 10px 0 4px 0; }
    h3 { font-size: 12pt; color: #3182ce; margin: 8px 0 3px 0; }
    p { margin: 2px 0 4px 0; text-align: justify; }
    ol, ul { margin: 2px 0 6px 16px; }
    li { margin-bottom: 0px; line-height: 1.2; }
    </style></head><body>
    """
...
def export_to_pptx(brief_text):
    ...
    slide.shapes.title.text = "Market Opportunity Brief"
    slide.placeholders[1].text = "Auto-generated with BriefForge"
...
if st.button("Generate Brief"):
    if industry and geography and goal and company_size and challenge:
        ...
        st.markdown("### 📊 Download Your Brief as PowerPoint")
        pptx_file = export_to_pptx(swot)
        st.download_button("📥 Download PPTX", pptx_file, file_name="Market_Opportunity_Brief.pptx")
    else:
